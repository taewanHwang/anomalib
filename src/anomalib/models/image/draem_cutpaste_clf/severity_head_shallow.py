...
class SeverityHead(nn.Module):
    ...
    Architecture (for 128x128 input):
        ...
        Flatten -> FC(5000) -> ReLU -> FC(1000) -> ReLU -> FC(100) -> ReLU -> FC(2)
    ...
    def __init__(
        ...
        self.fc1 = nn.Linear(self.flatten_size, 5000)  # 49152 -> 5000
        self.fc2 = nn.Linear(5000, 1000)                # 5000 -> 1000
        self.fc3 = nn.Linear(1000, 100)                 # 1000 -> 100
        self.fc4 = nn.Linear(100, 2)                    # 100 -> 2
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.fc1(x)         # (B, 49152) -> (B, 5000)
        x = self.relu(x)

        x = self.fc2(x)         # (B, 5000) -> (B, 1000)
        x = self.relu(x)

        x = self.fc3(x)         # (B, 1000) -> (B, 100)
        x = self.relu(x)

        x = self.fc4(x)         # (B, 100) -> (B, 2)
    ...
    def get_config(self) -> dict:
        ...
        return {
            ...
            "num_classes": 2,
            "conv_channels": [32, 48],
            "fc_layers": [self.flatten_size, 5000, 1000, 100, 2],
            "kernel_sizes": [5, 5],
