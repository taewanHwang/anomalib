"""FE-CLIP Training & Evaluation Script.

Optimal settings from Exp1-13 are fixed:
- tap_indices: [20, 21, 22, 23] (last 4 blocks)
- fc_patch lr: adapter_lr / 100
- evaluation: macro-average (category-wise → mean)
...
# Fixed optimal settings (from Exp1-13)
IMAGE_SIZE = 336
TAP_INDICES = [20, 21, 22, 23]  # last 4 blocks
TEMPERATURE = 0.07
EPOCHS = 9
...
class CLIPStyleDatasetWrapper(torch.utils.data.Dataset):
    """CLIP-style preprocessing: resize + center crop."""
...
def collect_test_data(dataset_class, root, categories):
    """Collect test data from all categories (paper protocol)."""
    all_datasets = []
...
def train(train_dataset, exp_dir: Path, device: str = "cuda:0"):
    """Train FE-CLIP with optimal settings."""
    ...
    # Optimal setting: fc_patch with 100x lower lr
    optimizer = torch.optim.Adam([
        {'params': adapter_params, 'lr': ADAPTER_LR},
        {'params': fc_patch_params, 'lr': FC_PATCH_LR}
    ])

    logger.info(f"Training with optimal settings:")
    logger.info(f"  tap_indices: {TAP_INDICES}")
...
def evaluate_macro(model, dataset_class, root, categories, device, transform):
    """Macro-average evaluation (paper method): category-wise metric → mean."""
    model.eval()
    ...
    for cat in categories:
        ...
        category_results[cat] = {
            ...
            "n_samples": len(cat_labels), "n_defect": sum(cat_labels),
        }
...
def main():
    parser = argparse.ArgumentParser(description="FE-CLIP Training & Evaluation")
    ...
    exp_name = args.exp_name or f"feclip_{args.mode}_seed{args.seed}"
    exp_dir = RESULTS_DIR / f"{exp_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    exp_dir.mkdir(parents=True, exist_ok=True)

    logger.info("=" * 80)
    logger.info(f"FE-CLIP Training & Evaluation")
    ...
    # Save results
    save_results = {k: v for k, v in results.items() if k != "per_category"}
    save_results["per_category"] = results["per_category"]
    save_results["seed"] = args.seed
    save_results["mode"] = args.mode
