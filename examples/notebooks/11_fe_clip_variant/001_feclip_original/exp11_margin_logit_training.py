"""Experiment 11: Margin-Logit Based Loss Training.

Instead of using softmax probability for mask loss,
use margin-logit (z·t_abn - z·t_norm) which provides stronger gradients
and better region discrimination.

Hypothesis: Margin-logit based loss may help the model learn
more discriminative anomaly maps, leading to better PRO.

Variants:
- baseline: prob_abnormal (softmax-based)
- margin: margin-logit (z·t_abn - z·t_norm)
- margin_sigmoid: sigmoid(margin-logit) for [0,1] range
"""
...
def margin_logit_loss(margin_map, gt_mask, temperature=1.0):
    """Compute loss using margin-logit map.

    For abnormal regions (gt=1): margin should be positive (high)
    For normal regions (gt=0): margin should be negative (low)

    Use BCE with logits where:
    - positive margin → predicted abnormal
    - negative margin → predicted normal
    """
    # margin_map is (z·t_abn - z·t_norm), no sigmoid applied yet
    # Use BCE with logits directly
    return F.binary_cross_entropy_with_logits(margin_map / temperature, gt_mask, reduction='mean')


def margin_focal_loss(margin_map, gt_mask, alpha=0.25, gamma=2.0, temperature=1.0):
    """Focal loss variant for margin-logit."""
    prob = torch.sigmoid(margin_map / temperature)

    # Focal weight
    pt = prob * gt_mask + (1 - prob) * (1 - gt_mask)
    ...
    bce = F.binary_cross_entropy_with_logits(
        margin_map / temperature, gt_mask, reduction='none'
    )
...
def train_with_margin_logit(
    ...
    lr: float = 5e-4,
    device: str = "cuda:0",
    loss_type: str = "margin",  # "baseline", "margin", "margin_sigmoid"
    temperature: float = 1.0,
):
    """Train FE-CLIP with margin-logit based loss."""
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # We need to access both probability and margin-logit
            scores, maps = model.model.forward_tokens(images)
            n_taps = len(scores)

            # Classification loss (unchanged - uses probability)
            ...
            if gt_mask is not None:
                for m in maps:
                    ...
                    if loss_type == "baseline":
                        # Original: use probability map with focal + dice
                        loss_mask = loss_mask + focal_loss(m_up, gt_mask) + dice_loss(m_up, gt_mask)
                    elif loss_type == "margin":
                        # Margin-logit: treat map as logits (before sigmoid)
                        # Convert probability back to logit for loss
                        # m_up is probability, so we need logit = log(p / (1-p))
                        eps = 1e-7
                        m_up_clipped = m_up.clamp(eps, 1 - eps)
                        logit = torch.log(m_up_clipped / (1 - m_up_clipped))
                        loss_mask = loss_mask + margin_focal_loss(logit, gt_mask, temperature=temperature)
                    elif loss_type == "margin_sigmoid":
                        # Same as margin but with additional dice loss
                        eps = 1e-7
                        m_up_clipped = m_up.clamp(eps, 1 - eps)
                        logit = torch.log(m_up_clipped / (1 - m_up_clipped))
                        loss_mask = loss_mask + margin_focal_loss(logit, gt_mask, temperature=temperature)
                        loss_mask = loss_mask + dice_loss(m_up, gt_mask)
...
def main():
    ...
    parser.add_argument("--loss-type", type=str, default="margin",
                        choices=["baseline", "margin", "margin_sigmoid"])
    parser.add_argument("--temperature", type=float, default=1.0)
    args = parser.parse_args()

    set_seed(args.seed)

    exp_name = f"exp11_{args.loss_type}_{args.mode}_seed{args.seed}"
    exp_dir = RESULTS_DIR / f"{exp_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    exp_dir.mkdir(parents=True, exist_ok=True)

    logger.info("=" * 80)
    logger.info(f"Experiment 11: Margin-Logit Based Loss Training")
    logger.info(f"Mode: {args.mode}, Loss Type: {args.loss_type}, Seed: {args.seed}")
    logger.info("=" * 80)
    ...
    # Train
    model = train_with_margin_logit(
        train_dataset, exp_dir, epochs=args.epochs, device=args.device,
        loss_type=args.loss_type, temperature=args.temperature
    )
    ...
    logger.info("\n" + "=" * 80)
    logger.info(f"RESULTS ({args.mode}, loss_type={args.loss_type}):")
