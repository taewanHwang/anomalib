"""Experiment 7: Abnormal-only mask loss training.

Hypothesis: Current implementation applies mask loss to ALL samples (including normal),
which might force the model to suppress the entire map when gt_mask=0.
This weakens contrast and specifically hurts PRO.

Solution: Only apply mask loss when gt_label==1 (abnormal samples).

Expected impact: VisA PRO should improve significantly (+5~15% if this is the cause).
"""
...
def train_with_abnormal_only_mask_loss(
    train_dataset,
    exp_dir: Path,
    epochs: int = 9,
    batch_size: int = 16,
    lr: float = 5e-4,
    device: str = "cuda:0",
    abnormal_only: bool = True,  # Key difference
):
    """Train FE-CLIP with abnormal-only mask loss."""
    ...
    logger.info(f"ABNORMAL_ONLY mask loss: {abnormal_only}")
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # Mask loss - KEY DIFFERENCE
            loss_mask = torch.tensor(0.0, device=device)
            if gt_mask is not None:
                if abnormal_only:
                    # Only apply mask loss on abnormal samples (gt_label == 1)
                    abnormal_mask_idx = (gt_label == 1).nonzero(as_tuple=True)[0]

                    if len(abnormal_mask_idx) > 0:
                        n_abnormal_batches += 1
                        for m in maps:
                            ...
                            m_up_abn = m_up[abnormal_mask_idx]
                            gt_mask_abn = gt_mask[abnormal_mask_idx]

                            loss_mask = loss_mask + focal_loss(m_up_abn, gt_mask_abn) + dice_loss(m_up_abn, gt_mask_abn)
                        loss_mask = loss_mask / n_taps
                else:
                    # Original: apply to all samples
                    for m in maps:
                        m_up = torch.nn.functional.interpolate(
                            m.unsqueeze(1), size=images.shape[-2:],
                            mode="bilinear", align_corners=False
        ...
        logger.info(f"Epoch {epoch+1}/{epochs}: loss={avg_loss:.4f} (cls={avg_cls:.4f}, mask={avg_mask:.4f}), "
                    f"abnormal batches: {n_abnormal_batches}/{len(train_loader)}")
...
def evaluate_model(model, dataset_class, root, categories, device, transform):
    ...
    # PRO - batch all defect samples together to avoid connected_components bug
    defect_preds = []
    defect_targets = []
    for pred, target in zip(all_maps, all_masks):
        if target.sum() > 0:
            defect_preds.append(torch.from_numpy(pred).unsqueeze(0).unsqueeze(0).float())
            defect_targets.append(torch.from_numpy(target).unsqueeze(0).unsqueeze(0).long())

    if len(defect_preds) > 0:
        all_preds_t = torch.cat(defect_preds, dim=0)
        all_targets_t = torch.cat(defect_targets, dim=0)
        aupro_metric = _AUPRO(fpr_limit=0.3)
        aupro_metric.update(all_preds_t, all_targets_t)
        pro = aupro_metric.compute().item() * 100
    else:
        pro = 0.0

    return {"AUROC": auroc, "AP": ap, "pAUROC": pauroc, "PRO": pro}
...
def main():
    ...
    parser.add_argument("--baseline", action="store_true", help="Run baseline (mask loss on all samples)")
    args = parser.parse_args()

    set_seed(args.seed)

    abnormal_only = not args.baseline
    exp_name = f"exp7_{'abnormal_only' if abnormal_only else 'baseline'}_{args.mode}_seed{args.seed}"
    exp_dir = RESULTS_DIR / f"{exp_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    exp_dir.mkdir(parents=True, exist_ok=True)

    logger.info("=" * 80)
    logger.info(f"Experiment 7: Abnormal-only mask loss")
    logger.info(f"Mode: {args.mode}, Abnormal-only: {abnormal_only}, Seed: {args.seed}")
    logger.info("=" * 80)
    ...
    # Train
    model = train_with_abnormal_only_mask_loss(
        train_dataset,
        exp_dir,
        epochs=args.epochs,
        device=args.device,
    ...
    logger.info("\n" + "=" * 80)
    logger.info(f"RESULTS ({args.mode}, abnormal_only={abnormal_only}):")
    ...
    logger.info(f"  PRO:         {results['PRO']:.1f}%")
    logger.info("=" * 80)
