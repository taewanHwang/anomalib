"""Benchmark spread_3 [15,18,21,23] tap configuration.

Full evaluation on MVTec, VisA, BTAD with 5 seeds each.
Uses optimal settings: fc_patch lr=1/100, macro-average evaluation.

Usage:
    CUDA_VISIBLE_DEVICES=0 python exp_spread3_benchmark.py --mode visa --seed 42
"""
...
RESULTS_DIR = SCRIPT_DIR / "results_spread3_benchmark"
...
# Best balanced 4-block configuration
TAP_INDICES = [15, 18, 21, 23]  # spread_3
...
def train(train_dataset, exp_dir: Path, device: str = "cuda:0"):
    """Train FE-CLIP with spread_3 configuration."""
...
def main():
    parser = argparse.ArgumentParser(description="Spread3 Benchmark")
    ...
    exp_name = f"spread3_{args.mode}_seed{args.seed}"
    exp_dir = RESULTS_DIR / f"{exp_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    exp_dir.mkdir(parents=True, exist_ok=True)

    logger.info("=" * 80)
    logger.info(f"Spread3 Benchmark: {TAP_INDICES}")
    ...
    logger.info("\n" + "=" * 80)
    logger.info(f"RESULTS: spread_3 {TAP_INDICES} on {args.mode.upper()}")
