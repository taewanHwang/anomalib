"""
Dinomaly TopK (Top-q% Loss) for HDMAP Dataset.

This script implements Method 2: Top-q% Loss variant of Dinomaly.
Top-q% Loss focuses on the highest distance values during training,
aligning the training objective with the evaluation metric (TPR@low FPR).
Inference is unchanged from baseline.

Key insight: The evaluation metric (TPR@FPR=1%) cares about the tail of
the score distribution. By training to focus on high-distance regions,
we improve separation at low FPR, particularly for weak defects (domain_C).

Key features:
- --q-percent: Top q% of distance values to use (default: 5.0)
- --q-schedule: Enable warmup schedule (100% -> q%)
- --warmup-steps: Number of warmup steps (default: 200)
- --seed: Specify random seed for reproducibility
- --result-dir: Specify output directory for results

Usage:
    python dinomaly_topk.py --seed 42 --gpu 0 --q-percent 5 --result-dir results/dinomaly_topk
...
from anomalib.models.image.dinomaly_variants import DinomalyTopK
...
def run_multiclass_experiment(
    ...
    """Run multi-class unified training experiment with DinomalyTopK.

    This uses Top-q% Loss for tail-focused training.
    ...
    **통합 데이터 로딩**: Training과 Testing 모두 동일한 HDMAPDataset을 사용합니다.
    ...
    Args:
        ...
        q_percent: Top q% of distance values to use (default: 5.0).
    ...
    logger.info("=" * 60)
    logger.info("MULTI-CLASS UNIFIED TRAINING (Top-q% Loss)")
    logger.info("=" * 60)
    logger.info(f"Seed: {seed}")
    logger.info(f"q_percent: {q_percent}, q_schedule: {q_schedule}, warmup_steps: {warmup_steps}")
    ...
    # Create model (DinomalyTopK with Top-q% Loss)
    model = DinomalyTopK(
...
def run_singleclass_experiments(
    ...
    """Run single-class (per-domain) training with DinomalyTopK.
    ...
    **통합 데이터 로딩**: HDMAPDataModule을 사용하여 Training과 Testing 동일한 방식 사용.
    ...
    """
    logger.info("=" * 60)
    logger.info("SINGLE-CLASS (PER-DOMAIN) TRAINING (Top-q% Loss)")
    logger.info("=" * 60)
    logger.info(f"Seed: {seed}")
    logger.info(f"q_percent: {q_percent}, q_schedule: {q_schedule}, warmup_steps: {warmup_steps}")
...
def main() -> None:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Dinomaly TopK (Method 2: Top-q% Loss) for HDMAP")
    parser.add_argument(
        "--mode",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.0,
        help="Top q%% of distance values to use for loss (default: 5.0)",
    )
    parser.add_argument(
        "--q-schedule",
        action="store_true",
        default=True,
        help="Enable warmup schedule (100%% -> q%%) (default: True)",
    )
    parser.add_argument(
        "--no-q-schedule",
        action="store_false",
        dest="q_schedule",
        help="Disable warmup schedule (use fixed q%% from start)",
    )
    parser.add_argument(
        "--warmup-steps",
        type=int,
        default=200,
        help="Number of warmup steps before reaching target q%% (default: 200)",
    )
    parser.add_argument(
        "--data-root",
        type=str,
    ...
    parser.add_argument(
        "--result-dir",
        type=str,
        default="results/dinomaly_topk",
        help="Result directory (default: results/dinomaly_topk)",
    )
    parser.add_argument(
        "--encoder",
        type=str,
    ...
    logger.info(f"q_percent: {args.q_percent}, q_schedule: {args.q_schedule}, warmup_steps: {args.warmup_steps}")
