"""
Dinomaly GEM (Generalized Mean Pooling) for HDMAP Dataset.
...
def run_multiclass_experiment(
    ...
    **통합 데이터 로딩**: Training과 Testing 모두 동일한 HDMAPDataset을 사용합니다.
    ...
    # Create model (DinomalyGEM with GEM pooling)
    model = DinomalyGEM(
...
def main() -> None:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Dinomaly GEM (Method 1) for HDMAP")
    parser.add_argument(
        "--mode",
        type=str,
    ...
    parser.add_argument(
        "--result-dir",
        type=str,
        default="results/dinomaly_gem",
        help="Result directory (default: results/dinomaly_gem)",
    )
    parser.add_argument(
        "--encoder",
        type=str,
